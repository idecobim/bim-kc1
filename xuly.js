...
/* ===== Việc chưa phân công (Cách 1: nhóm ảo, không thêm cột) ===== */
const CHUA_PC = '\u27e8chua-phan-cong\u27e9';   /* giá trị nội bộ để lọc, không trùng tên thật */
function chuaPhanCong(v){ return !String(v.nguoi||'').trim(); }
...
/* Chọn thẻ: click = chọn 1 · Ctrl/Cmd = bật/tắt từng thẻ · Shift = chọn dải (giữ qua các lần vẽ lại nhờ chonUids) */
...
document.querySelector('.kanban-wrapper').addEventListener('click', e=>{
  ...
  const tatCaUid = [...document.querySelectorAll('.kanban-wrapper .k-card')].map(c=>Number(c.dataset.uid));
  if(e.shiftKey && _kAnchor!=null){
    const i1=tatCaUid.indexOf(_kAnchor), i2=tatCaUid.indexOf(uid);
    if(i1>-1 && i2>-1){ const [a,b]=i1<i2?[i1,i2]:[i2,i1]; if(!(e.ctrlKey||e.metaKey)) chonUids.clear(); for(let k=a;k<=b;k++) chonUids.add(tatCaUid[k]); }
  } else if(e.ctrlKey || e.metaKey){
    if(chonUids.has(uid)) chonUids.delete(uid); else chonUids.add(uid);
    _kAnchor = uid;
  } else {
    chonUids.clear(); chonUids.add(uid); _kAnchor = uid;
  }
  document.querySelectorAll('.kanban-wrapper .k-card').forEach(c=>c.classList.toggle('k-selected', chonUids.has(Number(c.dataset.uid))));
});
...
document.addEventListener('contextmenu', e=>{
  ...
  const brTab2 = e.target.closest('#thCay .th-br');
  if(brTab2 && brTab2.dataset.mo){
    const parts = brTab2.dataset.mo.split('/').filter(Boolean);
    const mada = parts[0];
    if(parts.length>=2 && (laAdmin() || laLeaderCuaDuAn(mada))){
      e.preventDefault(); e.stopPropagation();
      _ctxBranchKey = brTab2.dataset.mo;
      const menu = $('thBranchMenu');
      menu.style.top = e.pageY + 'px'; menu.style.left = e.pageX + 'px';
      menu.classList.add('show');
    }
    return;
...
function moFormSuaViec(uid){
  ...
  if(typeof setupMauForm==='function') setupMauForm(true);   /* việc cũ: mặc định Tự do, giữ nguyên */
...
$('luoiDuAn').addEventListener('click', e=>{
  ...
  if($('btnQuanLyMau')) $('btnQuanLyMau').hidden = !laAdmin();
...
$('btnGiaoViecKanban').addEventListener('click', ()=>{
  ...
  if(typeof setupMauForm==='function') setupMauForm(false);   /* tạo mới: áp khuôn mẫu nếu dự án có */
...
$('guiViec').addEventListener('click', async ()=>{
  ...
  if(!v('cv-phancap') && !v('cv-nhiemvu')){ msg.className='modal-msg err'; msg.textContent='✖ Cần Phân cấp hoặc Nội dung công việc'; return; }
  ...
  if(ok){
    ...
    ['cv-phancap','cv-nhiemvu','cv-ghichu','cv-vuongmac','cv-tamngung'].forEach(id=>$(id).value='');
    if(typeof setupMauForm==='function') setupMauForm(false);   /* dựng lại khuôn cho việc kế tiếp */
...
function moQuanLyMau(mada){
  ...
  veDanhSachMau();
  moOverlay('modalQuanLyMau');
}
function veDanhSachMau(){
  const ds = mauCuaDuAn(_mauMada);
  $('qm-list').innerHTML = ds.length ? ds.map((m,i)=>
    '<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;border:1px solid var(--line);border-radius:6px;margin-bottom:8px">'
    +'<div style="flex:1;min-width:0"><div style="font-weight:600;font-size:14px">'+thoatHTML(m.ten)+'</div>'
    +'<div style="font-family:\'IBM Plex Mono\',monospace;font-size:11px;color:var(--concrete);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+thoatHTML(tomTatMau(m))+'</div></div>'
    +'<button class="th-nut" type="button" data-suamau="'+i+'">Sửa</button>'
    +'<button class="th-nut" type="button" data-xoamau="'+i+'" style="color:var(--red);border-color:var(--red)">Xóa</button></div>'
  ).join('') : '<div style="color:var(--concrete);font-size:13px;padding:10px 0">Dự án chưa có mẫu nào. Bấm "＋ Thêm mẫu".</div>';
  $('qm-list').querySelectorAll('[data-suamau]').forEach(b=>b.addEventListener('click',()=>moBuilderMau(mauCuaDuAn(_mauMada)[+b.dataset.suamau])));
  $('qm-list').querySelectorAll('[data-xoamau]').forEach(b=>b.addEventListener('click',()=>xoaMau(mauCuaDuAn(_mauMada)[+b.dataset.xoamau])));
}
...
async function luuMau(){
  ...
  try{
    const res=await fetch(LINK_APPS_SCRIPT,{method:'POST',body:JSON.stringify({type:'luumau',matkhau:maGui(),
      data:{ mada:_mauMada, ten, tengoc:(_mauEdit?_mauEdit.ten:''), levels }})});
    const kq=await res.json();
    if(kq.ok){
      if(_mauEdit){ const m=MAU_DS.find(x=>x.mada===_mauMada && x.ten===_mauEdit.ten); if(m){ m.ten=ten; m.levels=levels; } }
      else MAU_DS.push({mada:_mauMada,ten,levels});
      baoToast('✔ Đã lưu mẫu lên Sheet','ok');
      $('qm-builder').hidden=true; veDanhSachMau();
    } else baoToast('✖ '+(kq.loi||'Lỗi'),'err');
  }catch(e){ baoToast('✖ '+e.message,'err'); }
  finally{ btn.disabled=false; }
}
async function xoaMau(m){
  if(!m || !confirm('Xóa mẫu "'+m.ten+'" của dự án '+_mauMada+'?')) return;
  try{
    const res=await fetch(LINK_APPS_SCRIPT,{method:'POST',body:JSON.stringify({type:'xoamau',matkhau:maGui(),data:{mada:_mauMada,ten:m.ten}})});
    const kq=await res.json();
    if(kq.ok){ MAU_DS=MAU_DS.filter(x=>!(x.mada===_mauMada && x.ten===m.ten)); baoToast('🗑 Đã xóa mẫu','ok'); veDanhSachMau(); }
    else baoToast('✖ '+(kq.loi||'Lỗi'),'err');
  }catch(e){ baoToast('✖ '+e.message,'err'); }
}
...
function datPhancapTuDo(tuDo){
  const inp=$('cv-phancap'); if(!inp) return;
  inp.readOnly = !tuDo;
  inp.style.background = tuDo ? '' : 'var(--paper)';
  inp.style.opacity = tuDo ? '' : '.7';
  inp.title = tuDo ? '' : 'Ô này do khuôn mẫu tự ghép — chọn "Tự do" nếu muốn gõ tay';
}
if($('cv-mau')) $('cv-mau').addEventListener('change', apMauForm);
if($('btnQuanLyMau')) $('btnQuanLyMau').addEventListener('click', ()=>{ if(madaKanbanHienTai) moQuanLyMau(madaKanbanHienTai); });


/* ============================================================
   #5 THÊM VIỆC NHANH TỪ BẢNG TỔNG HỢP (chuột phải nhánh)
   #6 XEM & XÓA TỪNG GHI CHÚ (leader/admin)
   ============================================================ */
let _ctxBranchKey = null;
/* Dò mẫu khớp với đường dẫn nhánh đã chọn (để phân cấp thuộc mẫu nếu trùng) */
function khopMauVoiDuong(mada, segs){
  const ds = mauCuaDuAn(mada);
  for(let ti=0; ti<ds.length; ti++){
    const chon=[], nhap={}; let nodes=ds[ti].levels||[], ok=true;
    for(let L=0; L<segs.length; L++){
      if(!nodes || !nodes.length){ ok=false; break; }
      let j = nodes.findIndex(n=>!n.i && String(n.l).trim()===String(segs[L]).trim());
      if(j<0) j = nodes.findIndex(n=>n.i);   /* nút "nhập" nhận mọi giá trị */
      if(j<0){ ok=false; break; }
      chon[L]=j; if(nodes[j].i) nhap[L]=segs[L];
      nodes = nodes[j].c||[];
    }
    if(ok && segs.length) return { mauIndex:ti, chon, nhap };
  }
  return null;
}
function moThemViecNhanh(mada, segs){
  if(!(laAdmin() || laLeaderCuaDuAn(mada))){ baoToast('Chỉ leader/admin được thêm việc','err'); return; }
  uidDangSua=null; gocDangSua=null;
  if($('xoaViec')) $('xoaViec').hidden=true;
  $('tieuDeFormCV').textContent='🗒 Khai báo công việc';
  $('guiViec').textContent='Ghi vào Sheets';
  $('cv-mada').value=mada;
  ['cv-phancap','cv-nhiemvu','cv-ghichu','cv-vuongmac','cv-tamngung'].forEach(id=>$(id).value='');
  $('cv-uutien').value='Trung bình'; $('cv-trangthai').value='Chưa bắt đầu';
  const fp=document.querySelector('#cv-han')._flatpickr; if(fp) fp.clear(); else $('cv-han').value='';
  datNguoiForm('');
  const km = khopMauVoiDuong(mada, segs);
  setupMauForm(false);
  if(km && $('cv-mau') && !$('cv-mau-group').hidden){     /* trùng mẫu → dùng khuôn, chọn sẵn đến nhánh */
    $('cv-mau').value=String(km.mauIndex);
    _mauChon=km.chon.slice(); _mauNhap=Object.assign({},km.nhap);
    veCascade();
  } else {                                                /* không trùng mẫu → tự do, điền sẵn đường dẫn */
    if($('cv-mau')) $('cv-mau').value='__free';
    apMauForm();
    $('cv-phancap').value = segs.join(' / ');
  }
  apQuyenFormViec(mada);
  $('cv-matkhau').value=maXacNhanCache;
  $('msgViec').textContent='';
  moOverlay('modalViec');
}
if($('ctx-them-viec')) $('ctx-them-viec').addEventListener('click', ()=>{
  dongCtx();
...
function vietKhop(v, fNguoi, fText){
  ...
  const okN = !fNguoi || (fNguoi===CHUA_PC ? chuaPhanCong(v) : v.nguoi.split(',').map(s=>s.trim()).includes(fNguoi));
...
function veTongHop(){
  ...
  if($('thNguoi').dataset.loaded !== '1'){
    ...
    $('thNguoi').add(new Option('⚠ Chưa phân công', CHUA_PC));
...
function veViecCuaToi(){
  ...
  if(selT.dataset.scope !== scope){
    ...
    if(laQuanLyChung()) selT.add(new Option('⚠ Chưa phân công', CHUA_PC));
  ...
  let viec = (nguoiCuaToi===CHUA_PC)
    ? NHIEM_VU.filter(v => chuaPhanCong(v) && xemDuocViecTab3(v.mada))
    : NHIEM_VU.filter(v => v.nguoi.split(",").map(s=>s.trim()).includes(nguoiCuaToi) && xemDuocViecTab3(v.mada));
  const tenHienToi = (nguoiCuaToi===CHUA_PC) ? 'việc chưa phân công' : thoatHTML(nguoiCuaToi);
  if(!viec.length){ wrap.innerHTML = '<div class="th-empty">Không có '+tenHienToi+'.</div>'; if($('toiLocDA')) $('toiLocDA').innerHTML='<option value="">Tất cả dự án</option>'; return; }
...
function capNhatQuyen(){
  document.body.classList.toggle('is-leader', laAdmin());
  document.body.classList.toggle('is-khach', laKhach());
  const npt = $('nutPhanTichSheet');
  if(npt) npt.hidden = !(laAdmin() && typeof LINK_SHEET_DASHBOARD!=='undefined' && LINK_SHEET_DASHBOARD);
}
